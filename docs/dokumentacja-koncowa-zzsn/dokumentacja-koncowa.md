...
* Analiza przestrzeni słownika pod kątem istotności wymiarów reprezentacji dla danych konceptów
  * wymiarom przestrzeni ukrytej słownika jest przypisywana miara istotności dla wybranego konceptu
  * w celu interwencji wybieramy wymiary o najwyższej istotności i skalujemy je uwzględniając ich średnią aktywację w zbiorze danych oraz siłę interwencji (parametr)
...
![Demonstracja obrazków otrzymanych w wyniku interwencji](./example.png)

## Przykładowa wizualizacja aktywacji wymiaru słownika

![Przykładowa wizualizacja istotności wymiaru słownika](./saliency.png)
